...
//   an item of the sequenceDynamic. Returns POINTS[7] if the tests are passed. 
//   Otherwise returns 0.
// **************************************************************************
int test7()
{
	sequenceTemp<int> testDup, testUnique;
	size_t i, j;
	...
	// put items in the sequenceList of duplicates
	cout << "Attaching ";
	for (i = 0; i < arrSize3; i++) {
		cout << items3[i] << " ";
		testDup.attach(items3[i]);
	}
	cout << "to sequenceList of duplicate items." << endl;
	// try to erase item not found
	cout << "Attempting to erase item not in sequenceList...";
	...
	cout << "passed." << endl;
	// remove items from sequenceList of duplicates
	...
	for (i = 0; i < arrSize3; i++) {
		if (testDup.erase_one(items3[i])) {
			...
			cout << "\n\tverifying remaining items in sequenceList...";
			// check that all remaining items are in sequence
			for (j = i + 1; j < arrSize3; j++) {
	...
	// put items in the sequenceList of unique items
	// check for removal from beginning
	cout << "Attaching ";
	for (i = 0; i < arrSize5; i++) {
		cout << items5[i] << " ";
		testUnique.attach(items5[i]);
	}
	cout << "to sequenceList of unique items." << endl;
	// remove all items from non-duplicate array
	cout << "Erasing unique items from beginning of sequenceList...";
	for (i = 0; i < arrSize5; i++) {
		if (testUnique.erase_one(items5[i])) {
			...
			cout << " verifying remaining items in sequenceList...";
			// check that all remaining items are in sequence
			for (j = i + 1; j < arrSize5; j++) {
			...
			cout << "item not found...failed." << endl;
			return 0;
		}
	}
	cout << "passed." << endl;
	...
	cout << "to sequenceList of unique items." << endl;
	// remove all items from non-duplicate array
	cout << "Erasing unique items from end of sequenceList...";
	for (i = arrSize5; i > 0; i--) {
		if (testUnique.erase_one(items5[i - 1])) {
			cout << "\n\tremoved " << items5[i - 1];
			// position cursor at beginning
			testUnique.start();
			// check size after removed
			if (testUnique.size() + 1 != i) {
				cout << "failed." << endl;
				return 0;
			}
			cout << " verifying remaining items in sequenceList...";
			// check that all remaining items are in sequence
			for (j = 0; j < testUnique.size(); j++) {
				if (testUnique.current() != items5[j]) {
					cout << "failed." << endl;
					return 0;
				}
	...
	cout << "to sequenceList of unique items." << endl;
	// remove all items from non-duplicate array
	cout << "Erasing unique item from middle of sequenceList...";
	int itemToRemove = arrSize5 / 2;
	testUnique.erase_one(items5[itemToRemove]);
	cout << "\n\tremoved " << items5[itemToRemove];
	cout << " verifying remaining items in sequenceList...";
	// check size after removed
	if (testUnique.size() != arrSize5 - 1) {
		cout << "failed." << endl;
		return 0;
	}
	// check that all remaining items are in sequence
	for (j = 0; j < testUnique.size(); j++) {
		// skip over removed item
		if (j == itemToRemove)
			continue;
		if (testUnique.current() != items5[j]) {
			cout << "failed." << endl;
