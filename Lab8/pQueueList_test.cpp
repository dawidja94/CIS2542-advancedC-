// FILE: pQueueList_test.cpp
// Written by: Michael Main (main@colorado.edu)
// Modified by: CREngland
// An interactive test program for the template PriorityQueue ADT.
#include <cctype>     // Provides toupper
#include <iostream>   // Provides cout and cin
#include <cstdlib>    // Provides EXIT_SUCCESS and size_t
#include "pQueueList.h"  // With Item defined as int
using namespace std;
...
void print_pqueue(PriorityQueue<double>);
// Postcondition: All items on the queue are displayed in order.  Since the
// parameter is a copy, it is destructively modified.

int main( ) {
    PriorityQueue<double> test;    // A PriorityQueue that we'll perform tests on
...
void print_menu( ) {
	...
	cout << " P   Print the items in the ordered queue" << endl;
...
void print_pqueue(PriorityQueue<double> pq) {
	while (pq.size() > 0) {
		cout << pq.get_front() << ' ';
	}
	cout << endl;
