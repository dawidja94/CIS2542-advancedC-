// FILE: pQueueList_exam.cpp
// Written by: Michael Main (main@colorado.edu)
// Modified by: CREngland
...
// also been emptied.
// **************************************************************************
bool correct(PriorityQueue<double>& test, size_t n, int items[])
// Postcondition: Some tests have been run on the test priority queue.
// If this priority queue has n items, and these items are items[0]
// through items[n-1] (in that order), then the function has printed
// "Test passed." to cout and returned true. Otherwise the function
...
int test2( )
{
    ...
    PriorityQueue<double> test, empty;
    PriorityQueue<double>* pq_ptr;
    size_t base_usage;
    int i;
    double next;
    ...
    pq_ptr = new PriorityQueue<double>;    
    for (i = 0; i < TEST_SIZE; i++)
...
// value is zero.
{
    ...
    cout << flush;
    PriorityQueue<double> copy1(test);
    ...
    test.insert(3, 25);
    test.insert(4, 10);
    PriorityQueue<double> copy2(test);
...
// value is zero.
{
    PriorityQueue<double> test;
    int items[4] = { 1, 2, 3, 4 };
    char *oldbytes = new char[sizeof(PriorityQueue<double>)];
    char *newbytes = new char[sizeof(PriorityQueue<double>)];
    int i;

    cout << "A. Testing that assignment operator works okay for empty queue...";
    cout << flush;
    PriorityQueue<double> copy1;
    copy1.insert(1,1);
    copy1 = test;
    if (!correct(copy1, 0, items)) return 0;
    ...
    cout << flush;
    memcpy(oldbytes, &test, sizeof(PriorityQueue<double>));
    test = test;
    memcpy(newbytes, &test, sizeof(PriorityQueue<double>));
    for (i=0; i < sizeof(PriorityQueue<double>); i++)
